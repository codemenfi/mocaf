...
def export_survey_trips(survey):
    ...
    worksheet.write(row, col, "Matkan tunniste")
    worksheet.write(row, col + 1, "Tarkastettu")
    worksheet.write(row, col + 2, "Matkan tarkoitus")
    worksheet.write(row, col + 3, "Lähtöaika")
    worksheet.write(row, col + 4, "Saapumisaika")
    worksheet.write(row, col + 5, "Lähtöpaikka")
    worksheet.write(row, col + 6, "Määränpää")
    worksheet.write(row, col + 7, "Matkan pituus")
    worksheet.write(row, col + 8, "Matkanosan tunniste")
    worksheet.write(row, col + 9, "Matkanosan pituus")
    worksheet.write(row, col + 10, "Matkanosan kulkutapa")

    row += 1

    for trip in trips:
        worksheet.write(row, col, trip.id)
        worksheet.write(row, col + 1, "Kyllä" if trip.approved else "Ei")
        worksheet.write(row, col + 2, purpose_texts.get(trip.purpose, ""))
        worksheet.write(row, col + 3, trip.start_time.strftime("%d.%m.%Y %H:%M"))
        worksheet.write(row, col + 4, trip.end_time.strftime("%d.%m.%Y %H:%M"))
        worksheet.write(row, col + 5, trip.start_municipality)
        worksheet.write(row, col + 6, trip.end_municipality)
        worksheet.write(row, col + 7, trip.length)
        row += 1

        for leg in trip.legs_set.all():
            worksheet.write(row, col + 8, leg.id)
            worksheet.write(row, col + 9, leg.trip_length)
            worksheet.write(
                row, col + 10, transport_mode_texts.get(leg.transport_mode, "")
            )
            row += 1
