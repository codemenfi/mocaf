        ...
        "trips": [
            {
                ...
                else trip.deleted_at.isoformat(),
                "legs": [
                    {
                        "__typename": "Leg",
                        "id": str(leg.id),
...
def test_device_directive_missing_token(
    graphql_client_query, contains_error, uuid, trip
):
...
def test_device_directive_missing_uuid(
    graphql_client_query, contains_error, uuid, token, trip
):
    response = graphql_client_query(
        """
        query($token: String!)
...
def test_device_directive_invalid_token(
    graphql_client_query, contains_error, uuid, token, trip
):
...
def test_device_directive_mocaf_disabled(
    graphql_client_query, disable_mocaf, contains_error, uuid, token, trip
):
    assert trip.device
...
def test_enable_mocaf_device_directive_missing(
    ...
    assert contains_error(
        response, message="Device exists, specify token with the @device directive"
    )
...
def test_disable_mocaf_separately(graphql_client_query_data, uuid, token):
    data = graphql_client_query_data(
        """
...
def test_trips_ordered_by_time(graphql_client_query_data, reverse):
    if reverse:
        start1, start2 = make_aware(datetime(2020, 3, 2), utc), make_aware(
            datetime(2020, 3, 1), utc
        )
    else:
        start1, start2 = make_aware(datetime(2020, 3, 1), utc), make_aware(
            datetime(2020, 3, 2), utc
        )
    ...
    assert response == {"trips": [{"id": str(trip.id)} for trip in expected_order]}
...
def test_register_device_already_registered(graphql_client_query, contains_error):
    ...
    response = graphql_client_query(
        ...
        variables={
            ...
            "accountKey": str(device2.account_key),
        },
    )
...
def test_add_user_answer_to_questions(
    graphql_client_query_data, uuid, token, partisipants
):
    partisipants
    data = graphql_client_query_data(
        """
...
@freeze_time("2023-07-15")
def test_add_leg(
    graphql_client_query_data, uuid, token, survey_trip, day_info, partisipants
):
    survey_trip
    day_info
    partisipants
    data = graphql_client_query_data(
        ...
        mutation($uuid: String!, $token: String!)
        @device(uuid: $uuid, token: $token) {
           pollAddLeg(tripId: 1, tripLength: 3000, transportMode: "walk")  {
            ok
            }
        }
        """,
...
@freeze_time("2023-07-15")
def test_mark_user_day_ready(
    graphql_client_query_data, uuid, token, day_info, partisipants
):
    day_info
    partisipants
    data = graphql_client_query_data(
