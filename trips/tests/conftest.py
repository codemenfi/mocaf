...
def emission_budget_level_bronze():
    return EmissionBudgetLevel.objects.create(
        identifier="bronze", carbon_footprint=30, year=2020
    )


@pytest.fixture(autouse=True)
def survey():
    return SurveyInfo.objects.create(
        start_day="2023-7-15",
        end_day="2023-7-18",
        days=3,
        max_back_question=3,
...
def partisipants(survey, device):
    return Partisipants.objects.create(
...
def day_info(partisipants):
    return DayInfo.objects.create(
        partisipant=partisipants, date="2023-07-15", approved=False, id=1
    )
